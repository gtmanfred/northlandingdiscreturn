...
async def test_suggestions_manufacturer_returns_distinct_sorted(db, client):
    ...
    await disc_repo.create(manufacturer="Innova", name="Boss", colors=["Blue"], input_date=date.today())
    await disc_repo.create(manufacturer="Discraft", name="Buzzz", colors=["Red"], input_date=date.today())
    await disc_repo.create(manufacturer="Innova", name="Wraith", colors=["Green"], input_date=date.today())
...
async def test_suggestions_color(db, client):
    ...
    disc_repo = DiscRepository(db)
    await disc_repo.create(manufacturer="Innova", name="Boss", colors=["Blue"], input_date=date.today())
    await disc_repo.create(manufacturer="Innova", name="Wraith", colors=["Blue"], input_date=date.today())
    await disc_repo.create(manufacturer="Discraft", name="Buzzz", colors=["Red"], input_date=date.today())
...
async def test_suggestions_color_unnests_individual_tags(db, client):
    # Each tag in a multi-color disc is suggested separately, not the combination.
    user = await make_user(db)
    disc_repo = DiscRepository(db)
    await disc_repo.create(
        manufacturer="MVP", name="Wave", colors=["black", "white"], input_date=date.today()
    )
    await disc_repo.create(
        manufacturer="Innova", name="Flag", colors=["white", "red", "blue"], input_date=date.today()
    )

    resp = await client.get("/suggestions?field=color", headers=auth_headers(user.id))
    assert resp.status_code == 200
    # distinct individual tags, case-insensitive sort
    assert resp.json() == ["black", "blue", "red", "white"]


async def test_owners_by_phone_matches_suffix(db, client):
    admin = await make_user(db, is_admin=True)
    owner_repo = OwnerRepository(db)
    await owner_repo.resolve_or_create(
        first_name="Roger", last_name="Smith", phone_number="+15551234567"
    )
    await owner_repo.resolve_or_create(
        first_name="Other", last_name="Person", phone_number="+15559990000"
    )

    resp = await client.get("/suggestions/owners-by-phone?digits=4567", headers=auth_headers(admin.id))
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == 1
    assert body[0]["first_name"] == "Roger"
    assert body[0]["last_name"] == "Smith"
    assert body[0]["phone_number"] == "+15551234567"


async def test_owners_by_phone_short_input_returns_empty(db, client):
    admin = await make_user(db, is_admin=True)
    owner_repo = OwnerRepository(db)
    await owner_repo.resolve_or_create(
        first_name="Roger", last_name="Smith", phone_number="+15551234567"
    )
    resp = await client.get("/suggestions/owners-by-phone?digits=567", headers=auth_headers(admin.id))
    assert resp.status_code == 200
    assert resp.json() == []


async def test_owners_by_phone_requires_admin(db, client):
    user = await make_user(db, is_admin=False)
    resp = await client.get("/suggestions/owners-by-phone?digits=4567", headers=auth_headers(user.id))
